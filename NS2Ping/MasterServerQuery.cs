...
public class MasterServerQuery
{
    ...
    private void SendQueryRequest(UdpClient client, IPEndPoint startFrom)
    {
        ...
        byte[] payload = [0x31, 0xFF];
        payload = Writer.AppendCString(payload, startFrom.ToString());
        payload = Writer.AppendCString(payload, @"\appid\4920");

        PingService._logger.LogInformation("Sending serverlist request to master, starting from: " + startFrom.ToString());
    ...
    public bool HandleResponse(byte[] data, UdpClient client)
    {
        ...
        {
            PingService._logger.LogWarning("Received Master response, but there is no request in flight");
            return false;
        }
        ...
        foreach (byte b in magic)
        {
            ...
            {
                PingService._logger.LogWarning("Invalid header from master response!");
                return false;
            }
        }
        if (data.Length % magic.Length != 0)
        {
            PingService._logger.LogWarning("Invalid master response length");
        }
        ...
        while (reader.Remaining() > 0)
        {
            ...
            {
                PingService._logger.LogInformation("Completed master query!");
