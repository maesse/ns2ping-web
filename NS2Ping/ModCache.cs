...
public sealed class ModCache
{
    ...
    private ModCache()
    {
        ...
        {
            PingService._logger.LogWarning("Exception while trying to read apikeys:\n" + ex);
        }
        ...
        {
            PingService._logger.LogWarning("Exception while trying to read modcache:\n" + ex);
        }
    }
    ...
    {
        PingService._logger.LogInformation("Saving Mod Cache");
    ...
    public async Task<string?> GetModName(uint id)
    {
        ...
        {
            PingService._logger.LogInformation("Retrieving modinfo for " + id);
            ...
            if (resp != null && resp.response != null && resp.response.publishedfiledetails != null)
            {
                ...
                if (list.Count != 1)
                {
                    PingService._logger.LogWarning("Got strange SteamWebAPI response.. " + list.Count);
                }
                else
                {
                    bool modified = false;
